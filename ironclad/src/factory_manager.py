...
import ironclad
from code_utils import clean_json_response as utils_clean_json, clean_code_content, validate_python_syntax
...
def generate_main_candidate(blueprint, components):
    ...
    response = ollama.chat(model=MODEL_NAME, messages=[
        {'role': 'system', 'content': "You output valid JSON only. All code fields must contain actual newline characters, not escaped sequences."},
...
def repair_main_candidate(candidate_code, error_logs, components, module_dir):
    ...
    response = ollama.chat(model=MODEL_NAME, messages=[
        {'role': 'system', 'content': "You are a Python code repair specialist. Return only corrected Python code with actual newlines."},
        {'role': 'user', 'content': repair_prompt}
    ])
    
    repaired_code = clean_code_content(response['message']['content'])
